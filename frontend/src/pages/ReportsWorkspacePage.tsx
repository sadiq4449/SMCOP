...
export function ReportsWorkspacePage() {
  ...
  const onCreate = async () => {
    if (!newSchoolId.trim()) {
      setError('School ID is required to create a report.')
      return
    }
    setCreateBusy(true)
    setError(null)
      ...
      <section className="rounded-2xl border border-muted-surface bg-surface p-4 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          <input
            ...
            className="min-w-[260px] flex-1 rounded-lg border border-muted-surface px-3 py-2 font-mono text-sm text-text-primary"
          />
          <input
            value={newQuarter}
            onChange={(e) => setNewQuarter(e.target.value)}
            placeholder="Quarter"
            className="w-32 rounded-lg border border-muted-surface px-3 py-2 text-sm text-text-primary"
          />
          <button
            type="button"
            disabled={createBusy}
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-text-primary">
                {selected.quarter} — <span className="font-mono text-sm">{selected.school_id}</span>
              </h2>
